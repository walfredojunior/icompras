...
async function lerMemoria(): Promise<{ pct: number; usadaMb: number; totalMb: number } | null> {
  try {
...
export async function conferirLimites(): Promise<{ status: string; detail: string }> {
  const [m] = await pool.query(
    "SELECT mem_pct, disco_pct, carga1 FROM vps_metric ORDER BY at DESC LIMIT 1",
  );
